...
export default function FeedbackSection({ scanResultData }) {
    ...
    return (
        <Card className="mt-4">
            ...
            <CardContent>
                {scanResultData?.has_feedback ? (
                    <>
                        <div className="bg-white border border-orange-200 rounded-lg p-3 shadow-sm space-y-3">
                            <div className="flex justify-between items-center">
                                <span className="text-gray-700 text-sm font-medium">Prediction:</span>
                                <Badge variant="outline" className="text-orange-700 border-orange-300">
                                    {scanResultData?.details?.prediction || "N/A"}
                                </Badge>
                            </div>
                            <div>
                                ...
                                <p className="text-sm text-gray-800 leading-relaxed bg-green-50 border border-green-200 rounded-md p-2">
                                    {scanResultData?.details?.actual_outcome || "N/A"}
                                </p>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-gray-700 text-sm font-medium">Confidence:</span>
                                <span className="text-sm font-semibold text-orange-800 flex items-center gap-1">
                                    <Activity className="w-4 h-4 text-orange-500" />
                                    {scanResultData?.details?.confidence
                                        ? `${scanResultData?.details?.confidence.toFixed(2)}%`
                                        : "N/A"}
                                </span>
                            </div>
                        </div>
